...
class GatedFusion(nn.Module):
    ...
    def __init__(self, d_model):
        ...
        # 方法一的可学习权重
        self.weight_y = nn.Parameter(torch.ones(1, 1, d_model))
        self.weight_m = nn.Parameter(torch.ones(1, 1, d_model))

        # 或者方法二的线性层
        # self.gate_linear = nn.Linear(2 * d_model, d_model)

    def forward(self, y, m_out):
        """
        Args:
            y: Transformer blocks 输出, shape [batch, seq_len, d_model]
            m_out: 记忆模块输出, shape 可能是 [batch, d_model] 或 [batch, mem_len, d_model]
        Returns:
            o: 融合后的输出, shape [batch, seq_len, d_model]
        """
        # 1. 适配记忆模块输出的形状
        if m_out.dim() == 2:
            # 全局记忆向量 [batch, d_model] -> [batch, seq_len, d_model]
            m_out = m_out.unsqueeze(1).repeat(1, y.size(1), 1)
        elif m_out.dim() == 3 and m_out.size(1) != y.size(1):
            # 如果记忆模块输出多个向量但数量与序列长度不符，取平均或最后一个
            m_out = m_out.mean(dim=1)  # 变成 [batch, d_model]
            m_out = m_out.unsqueeze(1).repeat(1, y.size(1), 1)

        # 2. 实现门控融合 (这里使用方法一)
        y_norm = self.weight_y * y
        m_norm = self.weight_m * m_out

        # 计算门控信号
        gate = torch.sigmoid(y_norm + m_norm)

        # 应用门控
        o = gate * y_norm + (1 - gate) * m_norm

        return o


class Encoder(nn.Module):
    def __init__(self, state_dim, obs_dim, n_block, n_embd, n_head, n_agent, encode_state, n_persistent=2,
                 long_mem_dim=None, mem_depth=2, device='cpu'):
        ...
        # 新增: Gated Fusion 层
        self.gated_fusion = GatedFusion(n_embd)
    ...
    def forward(self, state, obs, long_mem_update=True):
        ...
        # 将长期记忆投影回n_embd维度
        longmem_emb = self.longmem_to_embd(longmem_out)  # [B, n_agent, n_embd] 或 [B, mem_len, n_embd]

        # ----------- 拼接送入主网络 ------------
        seq = torch.cat([persistent, x], dim=1)  # [B, N_persist + n_agent, n_embd]

        # 整个序列跑一遍 encoder
        rep_all = self.blocks(self.ln(seq))  # [B, N_persist + n_agent, n_embd]

        # ----------- 门控融合 ------------
        # 将Transformer blocks输出rep_all与长期记忆longmem_emb进行融合
        # 注意: 我们需要将长期记忆与rep_all中对应的部分进行融合
        # 首先，将长期记忆扩展到与rep_all相同的序列长度
        if longmem_emb.dim() == 2:
            # 全局记忆向量 [batch, n_embd] -> [batch, N_persist + n_agent, n_embd]
            longmem_expanded = longmem_emb.unsqueeze(1).repeat(1, rep_all.size(1), 1)
        elif longmem_emb.dim() == 3 and longmem_emb.size(1) == self.n_agent:
            # 长期记忆有n_agent个token [batch, n_agent, n_embd]
            # 我们需要在前面添加N_persist个零向量，使其与rep_all对齐
            N_p = persistent.size(1)
            batch_size = rep_all.size(0)
            device = rep_all.device

            # 创建前缀零向量
            prefix = torch.zeros(batch_size, N_p, self.n_embd, device=device)

            # 拼接前缀和长期记忆
            longmem_expanded = torch.cat([prefix, longmem_emb], dim=1)
        else:
            # 其他情况，直接复制到相同长度
            longmem_expanded = longmem_emb.repeat(1, rep_all.size(1) // longmem_emb.size(1), 1)

        # 进行融合
        fused_rep = self.gated_fusion(rep_all, longmem_expanded)  # [B, N_persist + n_agent, n_embd]

        # ----------- 通过head计算价值 ------------
        v_loc_all = self.head(fused_rep)  # [B, N_persist + n_agent, 1]

        # 切片，只保留 obs 那 n_agent 个 token 的输出
        N_p = persistent.size(1)
        start = N_p
        rep = fused_rep[:, start:, :]  # [B, n_agent, n_embd]
...
#         seq = torch.cat([persistent, x], dim=1)   # [B, N_persist + n_agent + n_agent, n_embd]
#
#         """
#         encoder后  不切片
#         """
...
#         v_loc = v_loc_all[:, start:, :]  # [B, n_agent,   1]
#         return v_loc, rep  # 多返回一个长期记忆的loss（训练/分析用）
...
class MultiAgentTransformer(nn.Module):
    ...
    def forward(self, state, obs, action, available_actions=None):
        ...
        v_loc, obs_rep,_= self.encoder(state, obs)
        if self.action_type == 'Discrete':
            action = action.long()
