...
class TitansMemoryStrict(nn.Module):
    def __init__(self, dim, depth=2, expansion=4.0, device='cpu'):
        ...
        init_state = self._flatten_memory_params()
        self.param_shapes = [tuple(p.shape) for p in self.memory.parameters()]
        self.param_numels = [p.numel() for p in self.memory.parameters()]

        # Fast memory state is a buffer, not an optimizer-owned parameter.
        self.register_buffer('memory_state', init_state.clone())
        self.register_buffer('S', torch.zeros_like(init_state))

    def _flatten_memory_params(self):
        return torch.cat([p.detach().reshape(-1) for p in self.memory.parameters()])

    def _memory_forward(self, x, flat_weights):
        start = 0

        for ind, (shape, numel) in enumerate(zip(self.param_shapes, self.param_numels)):
            if ind > 0:
                x = F.gelu(x)

            weight = flat_weights[start:start + numel].view(shape)
            x = x @ weight
            start += numel
    ...
    def forward(self, key, value, update=True):
        state_for_read = self.memory_state.detach().clone()
        out = self._memory_forward(key, state_for_read)
        loss = self.loss_fn(out, value)

        if update and self.training:
            key_for_update = key.detach()
            value_for_update = value.detach()
            state_for_update = self.memory_state.detach().clone().requires_grad_(True)

            pred = self._memory_forward(key_for_update, state_for_update)
            update_loss = self.loss_fn(pred, value_for_update)
            grad_vec, = torch.autograd.grad(update_loss, state_for_update, retain_graph=False, create_graph=False)

            alpha_t = self.alpha_head(key_for_update).mean().detach()
            eta_t   = self.eta_head(key_for_update).mean().detach()
            theta_t = self.theta_head(key_for_update).mean().detach()

            S_new = eta_t * self.S - theta_t * grad_vec
            with torch.no_grad():
                self.memory_state.mul_(1. - alpha_t).add_(S_new)
                self.S.copy_(S_new)
    ...
    def reset_memory_state(self):
        self.memory_state.copy_(self._flatten_memory_params())
        self.reset_momentum()
...
class Encoder(nn.Module):
    ...
    def forward(self, state, obs, long_mem_update=True):
        ...
        longmem_out = self.longmem_to_embd(longmem_out)
