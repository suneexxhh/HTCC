...
def print_config(all_args):
    """Print all training configuration values in a stable order."""
    config = vars(all_args)
    key_width = max((len(key) for key in config), default=0)
    print("=" * 80)
    print("Training configuration")
    print("=" * 80)
    for key in sorted(config):
        print(f"{key:<{key_width}} : {config[key]}")
    print("=" * 80)
...
def get_config():
    ...
    Prepare parameters:
        --algorithm_name <algorithm_name>
            specifiy the algorithm, including `["memat", "nest_mat_lite", "nest_mat_clock", "nest_mat", "mat_dec", "memat_episode"]`
        --experiment_name <str>
    ...
    parser.add_argument("--algorithm_name", type=str,
                        default='memat', choices=["memat", "nest_mat_lite", "nest_mat_clock", "nest_mat", "memat_episode", "mat_dec", "mat_encoder", "mat_decoder", "mat_gru"])
    ...
    parser.add_argument("--nest_cms_levels", type=int, default=3,
                        help="Number of CMS-style team memory levels for NEST-MAT-Lite.")
    parser.add_argument("--nest_cms_hidden_mult", type=int, default=2,
                        help="Hidden multiplier for NEST-MAT-Lite CMS residual blocks.")
    parser.add_argument("--nest_gate_bias", type=float, default=-2.0,
                        help="Initial gate bias for weak NEST-MAT-Lite memory injection.")
    parser.add_argument("--nest_alpha_init", type=float, default=0.05,
                        help="Initial residual scale for NEST-MAT-Lite memory injection.")
    parser.add_argument("--nest_delta_clip", type=float, default=1.0,
                        help="Norm clip for NEST-MAT-Lite memory residuals.")
    parser.add_argument("--nest_clock_periods", type=str, default="1,2,4",
                        help="Comma-separated fast/mid/slow update periods for NEST-MAT-Clock.")
    parser.add_argument("--nest_clock_mode", type=str, default="soft", choices=["hard", "soft"],
                        help="Clock mode for NEST-MAT-Clock feature updates.")
    parser.add_argument("--nest_clock_min_rate", type=float, default=0.25,
                        help="Minimum soft-clock update rate for NEST-MAT-Clock.")
    parser.add_argument("--nest_clock_warmup", type=int, default=0,
                        help="Hard-clock warmup steps before non-initial level updates.")
    parser.add_argument("--nest_clock_beta", type=float, default=1.0,
                        help="Clock strength in [0,1]. beta=0 gives Lite-like full-rate CMS levels; beta=1 gives the current NEST-MAT-Clock.")
