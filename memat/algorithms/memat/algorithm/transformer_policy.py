import torch
import numpy as np
from memat.utils.util import update_linear_schedule
from memat.utils.util import get_shape_from_obs_space, get_shape_from_act_space
from memat.algorithms.utils.util import check
from memat.algorithms.memat.algorithm.ma_transformer import MultiAgentTransformer
...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        if self.algorithm_name in ["memat", "mat_dec"]:
            from memat.algorithms.memat.algorithm.ma_transformer import MultiAgentTransformer as MAT
        elif self.algorithm_name in ["nest_mat_lite", "nest_mat"]:
            from memat.algorithms.memat.algorithm.ma_transformer_nest import MultiAgentTransformer as MAT
        elif self.algorithm_name == "nest_mat_clock":
            from memat.algorithms.memat.algorithm.ma_transformer_nest_clock import MultiAgentTransformer as MAT
        elif self.algorithm_name == "memat_episode":
            from memat.algorithms.memat.algorithm.ma_transformer_episode import MultiAgentTransformer as MAT
        elif self.algorithm_name == "mat_gru":
            from memat.algorithms.memat.algorithm.mat_gru import MultiAgentGRU as MAT
        elif self.algorithm_name == "mat_decoder":
            from memat.algorithms.memat.algorithm.mat_decoder import MultiAgentDecoder as MAT
        elif self.algorithm_name == "mat_encoder":
            from memat.algorithms.memat.algorithm.mat_encoder import MultiAgentEncoder as MAT
        else:
            raise NotImplementedError

        if self.algorithm_name == "memat_episode":
            self.transformer = MAT(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
                                   ...
                                   episode_mem_dim=args.hidden_size,
                                   episode_recurrent_N=args.recurrent_N)
        elif self.algorithm_name in ["nest_mat_lite", "nest_mat"]:
            self.transformer = MAT(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
                                   n_block=args.n_block, n_embd=args.n_embd, n_head=args.n_head,
                                   encode_state=args.encode_state, device=device,
                                   action_type=self.action_type, dec_actor=args.dec_actor,
                                   share_actor=args.share_actor,
                                   nest_cms_levels=args.nest_cms_levels,
                                   nest_cms_hidden_mult=args.nest_cms_hidden_mult,
                                   nest_gate_bias=args.nest_gate_bias,
                                   nest_alpha_init=args.nest_alpha_init,
                                   nest_delta_clip=args.nest_delta_clip)
        elif self.algorithm_name == "nest_mat_clock":
            self.transformer = MAT(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
                                   n_block=args.n_block, n_embd=args.n_embd, n_head=args.n_head,
                                   encode_state=args.encode_state, device=device,
                                   ...
                                   nest_alpha_init=args.nest_alpha_init,
                                   nest_delta_clip=args.nest_delta_clip,
                                   nest_clock_periods=args.nest_clock_periods,
                                   nest_clock_mode=args.nest_clock_mode,
                                   nest_clock_min_rate=args.nest_clock_min_rate,
                                   nest_clock_warmup=args.nest_clock_warmup,
                                   nest_clock_beta=args.nest_clock_beta)
        else:
            self.transformer = MAT(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
                                   n_block=args.n_block, n_embd=args.n_embd, n_head=args.n_head,
                                   encode_state=args.encode_state, device=device,
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        if getattr(self.transformer, "uses_episode_memory", False):
            episode_state = check(rnn_states_actor).to(**self.tpdv)
            episode_masks = check(masks).to(**self.tpdv)
            actions, action_log_probs, values, next_episode_state = self.transformer.get_actions(
                cent_obs, obs, available_actions, deterministic,
                episode_state=episode_state, masks=episode_masks
            )
        else:
        ...
        if getattr(self.transformer, "uses_episode_memory", False):
            rnn_states_actor = next_episode_state
            rnn_states_critic = next_episode_state.clone()
        else:
    ...
    def get_values(self, cent_obs, obs, rnn_states_critic, masks, available_actions=None):
        ...
        if getattr(self.transformer, "uses_episode_memory", False):
            episode_state = check(rnn_states_critic).to(**self.tpdv)
            episode_masks = check(masks).to(**self.tpdv)
    ...
    def get_memory_stats(self):
        get_stats = getattr(self.transformer, "get_memory_stats", None)
