...
class Encoder(nn.Module):
    ...
    def forward(self, state, obs, long_mem_update=True):
        ...
        if self.encode_state:
            state_embeddings = self.state_encoder(state)  # Process state through state encoder
            x = state_embeddings
        else:
            obs_embeddings = self.obs_encoder(obs)  # Process observation through observation encoder
            x = obs_embeddings
        ...
        # ----------- 公式 30: 通过长期记忆模块处理拼接后的序列 ------------
        mem_key = self.mem_proj(seq)  # [B, N_persist + n_agent, mem_dim]
        mem_value = self.value_proj(seq)  # [B, N_persist + n_agent, mem_dim]

        # 通过长期记忆模块更新记忆并得到输出
        longmem_out, mem_loss = self.longterm_mem(mem_key, mem_value,
                                                  update=long_mem_update)  # [B, N_persist + n_agent, mem_dim]

        # ----------- 将长期记忆输出转换回嵌入维度 ------------
        y = self.longmem_to_embd(longmem_out)  # [B, N_persist + n_agent + n_agent, n_embd]

        # 残差连接：加上原始输入seq
        y = y + seq
        # ----------- 通过attention模块进一步处理y ------------
        rep_all = self.blocks(self.ln(y))  # [B, N_persist + n_agent + n_agent, n_embd] (通过 blocks 处理 y)
        v_loc_all = self.head(rep_all)  # [B, N_persist + n_agent + n_agent, 1] (最终输出 o)
